...
public class GameController {
    ...
    public String tryToMakeBuilding(Home buildingHome, String buildingName, boolean[] isSuccessful) {
        ...
        decreaseElementFromStockPlie("wood", neededWood);
    ...
    public String createUnit(Matcher matcher) {
        ...
        isSuccessful[0] = false;
        int i;
        for (i = 0; i < count && i < getCurrentGovernment().getPopulation(); i++) {
    ...
    public String tryToCreateUnit(Home targetHome, String troopName, boolean[] isSuccessful) {
        ...
        isSuccessful[0] = true;
        getCurrentGovernment().setPopulation(getCurrentGovernment().getPopulation() - 1);
    ...
    private void decreaseElementFromStockPlie(String elementType, double amount) {
        ...
        for (int i = 0; i < getCurrentGovernment().getStockPiles().size(); i++) {
            ...
            if (decreaseAmount <= 0) {
                stockPile.setCapacity(stockPile.getCapacity() + amount);
                stockPile.setElements(elementType, stockPile.getElements().get(elementType) - amount);
                return;
            } else {
                stockPile.setCapacity(stockPile.getCapacity() + stockPile.getElements(elementType));
    ...
    private void decreaseElementFromArmory(String elementType, int amount) {
        ...
        for (int i = 0; i < getCurrentGovernment().getArmouries().size(); i++) {
            ...
            if (decreaseAmount <= 0) {
                armoury.setCapacity(armoury.getCapacity() + amount);
                armoury.setElements(elementType, armoury.getElements(elementType) - amount);
                return;
            } else {
                armoury.setCapacity(armoury.getCapacity() + armoury.getElements(elementType));
...
//                    continue;
                if (targetHome == null)
                    continue;
                if (!home.getOwner().equals(targetHome.getOwner()))
    ...
    public String attackToEnemy(Matcher matcher) {
        ...
        Building targetBuilding = enemyHome.getBuilding();
        if (enemyTroops.size() == 0 && targetBuilding == null)
    ...
    private void setAttackBetweenEnemyAndYours(ArrayList<Troop> yourTroops, ArrayList<Troop> enemyTroops) {
        ...
        if (yourTroops.size() == 0 || enemyTroops.size() == 0)
            return;
        if (yourTroops.size() >= enemyTroops.size()) {
            for (int j = 0; j < enemyTroops.size(); j++) {
                reduceFromTroopHitpoint(yourTroops.get(j), enemyTroops.get(j));
                reduceFromTroopHitpoint(enemyTroops.get(j), yourTroops.get(j));
            }
            for (int j = 0; j < yourTroops.size() - enemyTroops.size(); j++) {
                reduceFromTroopHitpoint(yourTroops.get(j), enemyTroops.get(j % enemyTroops.size()));
            }
            ...
            for (int j = 0; j < enemyTroops.size() - yourTroops.size(); j++) {
                reduceFromTroopHitpoint(enemyTroops.get(j), yourTroops.get(j % yourTroops.size()));
            }
        }
    }
    ...
    public String engineAttack(Matcher matcher) {
        ...
        if (getCurrentGame().getSelectEngineBuildingHome() == null)
            return "Please choose a engineBuilding first";
        ...
        if (engineName.equals("MovableManjenigh") || engineName.equals("StopManjenigh") || engineName.equals("FireStoneDropper") || engineName.equals("DejKoob")) {
    ...
    public String disbandUnit() {
        ...
        getCurrentGovernment().setPopularity(getCurrentGovernment().getPopulation() + populationIncrease);
    ...
    public String dropUnit(Matcher matcher) {
        ...
        boolean[] isSuccessful = new boolean[1];
        String tryOutput;
        for (int i = 0; i < troopCount && i < getCurrentGovernment().getPopulation(); i++) {
    ...
    private void updateOilEngineersBeforeTurn() {
        ArrayList<Troop> yourAllTroops = getCurrentGovernment().getTroops();
        if (yourAllTroops == null)
            return;
        Home home;
        for (int i = 0; i < yourAllTroops.size(); i++) {
            if (yourAllTroops.get(i).getType().equals("Engineer") && yourAllTroops.get(i).getHome().getBuilding().getType().equals("OilSmelter")) {
                home = yourAllTroops.get(i).getHome();
                if (yourAllTroops.get(i).getState().equals("offensive")) {
    ...
    private void updateAngryTroopBeforeTurn() {
        ...
        for (int i = 0; i < allHomes.size(); i++) {
            ...
            if (thisHomeEnemyTroops.size() == 0)
                continue;
            allEnemyTroops.addAll(thisHomeEnemyTroops);
        }

        if (angryTroops.size() < allEnemyTroops.size()) {
            ArrayList<Troop> targetTroops = new ArrayList<>();
            for (int i = 0; i < angryTroops.size(); i++) {
                targetTroops.add(allEnemyTroops.get(i));
            }
            setAttackBetweenEnemyAndYours(angryTroops, targetTroops);
        } else {
            setAttackBetweenEnemyAndYours(angryTroops, allEnemyTroops);
        }
    ...
    private void updatePopularityAndPopulationAfterTurn() {
        ...
        mustBeAddedPopularity += getTheFoodPopularityEffectPerOne(foodRate);
        mustBeAddedPopularity += getTheTaxPopularityEffectPerOne(taxRate);
    ...
    private void updateGraneriesAfterTurn() {
        setFoodRateEffect();
        setTheFarmEffect();
    }
    ...
    private Double reduceRandomFoodFromAGranery(Granery granery, Double amountOfDecrease) {
        ...
        for (int i = 0; i < Granery.foodNames.size(); i++) {
            ...
            if (existFoodAmount > amountOfDecrease) {
                granery.setCapacity(granery.getCapacity() + amountOfDecrease);
                granery.setElements(Granery.foodNames.get(i), existFoodAmount - amountOfDecrease);
                amountOfDecrease = -1.0;
            } else {
                granery.setCapacity(granery.getCapacity() + granery.getElements(Granery.foodNames.get(i)));
    ...
    private Integer updateStockPilesAfterTurn() {
    ...
    private void increaseElementOfStockPile(String productName, int amount) {
        ...
        for (int i = 0; i < allStockpiles.size(); i++) {
            ...
            if (amount > allStockpiles.get(i).getCapacity()) {
                allStockpiles.get(i).setElements(productName, allStockpiles.get(i).getCapacity() + allStockpiles.get(i).getElements(productName));
                allStockpiles.get(i).setCapacity(0.0);
            } else {
                System.out.println(";;;;;;;");
                allStockpiles.get(i).setElements(productName, amount + allStockpiles.get(i).getElements(productName));
                allStockpiles.get(i).setCapacity(allStockpiles.get(i).getCapacity() - amount);
            }
        }
    }

    private void increaseElementOfArmoury(String productName, int amount) {
        ArrayList<Armoury> armouries = getCurrentGovernment().getArmouries();
        for (int i = 0; i < armouries.size(); i++) {
            if (amount < 0)
                return;
            if (amount > armouries.get(i).getCapacity()) {
                armouries.get(i).setElements(productName, armouries.get(i).getCapacity() + armouries.get(i).getElements(productName));
                armouries.get(i).setCapacity(0.0);
            } else {
                armouries.get(i).setElements(productName, amount + armouries.get(i).getElements(productName));
                armouries.get(i).setCapacity(armouries.get(i).getCapacity() - amount);
            }
        }
    }
    ...
    private void updateArmouryAfterTurn(Integer availableCounter) {
        ...
        ArrayList<Home> allHomes = getCurrentGame().getMap().getHomes();
        for (int i = 0; i < allHomes.size(); i++) {
            if (!allHomes.get(i).getOwner().equals(getCurrentGovernment()))
                continue;
        ...
        for (int i = 0; i < allMine.size() && i < availableCounter; i++) {
            increaseElementOfArmoury(allMine.get(i).getProductionName(), allMine.get(i).getProductionAmount());
        }
    }


    private void updateTroopsAfterTurn() {
        ArrayList<Home> allHomes = getCurrentGame().getMap().getHomes();
    ...
    private void reduceAllTroopsInBadFloor(ArrayList<Home> allHome) {
        ...
        for (int i = 0; i < allHome.size(); i++) {
            ...
            for (int j = 0; j < thisHomeAllTroops.size(); j++) {
                thisHomeAllTroops.get(j).setHitpoint(thisHomeAllTroops.get(j).getHitpoint() - mustBeDecreaseFromHitpoint);
            }
        }
    }
    ...
    public String showHomeInfo(Matcher matcher) {
        Integer x = Integer.parseInt(matcher.group("x"));
        Integer y = Integer.parseInt(matcher.group("y"));
        Home targetHome = mapController.getHomeByPosition(x, y);
        if (targetHome == null)
            return "In valid home pos";
        StringBuilder out = new StringBuilder("Home < " + x + " , " + y + "> : \n");
        ArrayList<Troop> yourAllTroops = returnYourTroopsFromAll(targetHome.getTroops());
        for (int i = 0; i < yourAllTroops.size(); i++) {
            out.append("Troop ").append(i + 1).append(" : ").append(" name --> ").append(yourAllTroops.get(i).getType()).append(" state --> ").append(yourAllTroops.get(i).getState()).append(" hitpoint --> ").append(yourAllTroops.get(i).getHitpoint()).append("\n");
        }
        if (targetHome.getBuilding() != null)
            out.append("Building : " + " name --> ").append(targetHome.getBuilding().getType()).append(" hitpoint --> ").append(targetHome.getBuilding().getHitpoint()).append("\n");
        return out.toString();
    }
}
