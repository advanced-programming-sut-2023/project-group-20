...
public class TradeController extends CheckController {
    ...
    public String tradeAccept(Matcher matcher) {
        ...
        if (!shopController.buy2(Trade.getTradeById(id).getType(), Trade.getTradeById(id).getAmount()).equals("The item purchased")) {
    ...
    public String tradeHistory() {
        ...
        for (int i = 0; i < Trade.trades.size(); i++) {
            if (Trade.trades.get(i).isFlag()) {
                txt += (Trade.trades.get(i).getId() + ") sender:" + Trade.trades.get(i).getReceiverOrSender().getOwner().getNickname()
                        + "," + Trade.trades.get(i).getType() + " :" + Trade.trades.get(i).getAmount() + ", price: "
                        + Trade.trades.get(i).getPrice() + "; message:" + Trade.trades.get(i).getMessage() + ".\n");
            }
        }
        if (txt.equals(""))
            txt = "you haven't any trade\n";
