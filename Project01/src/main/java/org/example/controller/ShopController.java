...
public class ShopController {
    ...
    public String showPriceList(){
        ...
        for (Armoury armoury: currentGovernment.getArmouries2() ) {
    ...
    public String buy(Matcher matcher){
        ...
        if( granery.contains(name) ){
            ...
            for(int i=0;i<currentGovernment.getArmouries2().size();i++){
                if(currentGovernment.getArmouries2().get(i).getCapacity()>amount){
                    currentGovernment.getArmouries2().get(i).changeCapacity((double) amount);
                    currentGovernment.getArmouries2().get(i).setElements(name, currentGovernment.getArmouries2().get(i).getElements(name)+amount );
                    break;
                }else{
                    currentGovernment.getArmouries2().get(i).setElements(name, currentGovernment.getArmouries2().get(i).getElements(name)+currentGovernment.getArmouries2().get(i).getCapacity() );
                    amount -= currentGovernment.getArmouries2().get(i).getCapacity();
                    currentGovernment.getArmouries2().get(i).setCapacity(0.0);
                }
            }
    ...
    public String sell(Matcher matcher){
        ...
        if( granery.contains(name) ){
            ...
            for(int i=0;i<currentGovernment.getArmouries2().size();i++){
                if(currentGovernment.getArmouries2().get(i).getElements(name)>amount){
                    currentGovernment.getArmouries2().get(i).changeCapacity((double) -amount);
                    currentGovernment.getArmouries2().get(i).setElements(name, currentGovernment.getArmouries2().get(i).getElements(name)-amount );
                    break;
                }else{
                    currentGovernment.getArmouries2().get(i).changeCapacity(-currentGovernment.getArmouries2().get(i).getElements(name));
                    amount -= currentGovernment.getArmouries2().get(i).getElements(name);
                    currentGovernment.getArmouries2().get(i).setElements(name, 0.0 );
                }
            }
        }else if( stockpile.contains(name) )
            for(int i=0;i<currentGovernment.getStockPiles().size();i++){
...
//        GameController.start();
    ...
    public Double armouryCapacity(Government government){
        Double tmp = 0.0;
        for (Armoury armoury: government.getArmouries2()) {
    ...
    public Double stockpilegood(Government government, String name){
        Double tmp = 0.0;
        for (StockPile stockPile: government.getStockPiles()) {
            tmp += stockPile.getElements(name);
        }
        return tmp;
    }
    ...
    public String buy2(String name, Double amount){
        Matcher matcher;
        String command = "buy -i " + name + " -a " + amount;//TODO: REGEX check shavad!
        matcher = isMatched(command, "^buy(((?: -a (?<amount>\\d+))|(?: -i (?<id>(?:\"[^\"]+\")|(?:(?!\")\\S+(?<!\"))))){0,1}){0,2}$");
        return buy(matcher);
    }

    public String sell2(String name, Double amount){
        Matcher matcher;
        String command = "sell -i " + name + " -a " + amount;//TODO: REGEX check shavad!
        matcher = isMatched(command, "^sell(((?: -a (?<amount>\\d+))|(?: -i (?<id>(?:\"[^\"]+\")|(?:(?!\")\\S+(?<!\"))))){0,1}){0,2}$");
        return buy(matcher);
    }
